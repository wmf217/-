              ...
              <div class="row">
                    <div class="col-sm-12">
                        <section class="panel panel-default">
                            <header class="panel-heading font-bold">编辑{{$title}}</header>
                            <div class="panel-body">
                                <form role="form" action="{{action($controller . "@update", [$matrix_id, $model->id])}}" method='POST' enctype="multipart/form-data">
                                    ...
                                    <select id="{{$field}}" name="{{$field}}" class="form-control">
                                        ...
                                        <option @if($model->$field==$k)selected="selected"@endif value ="{{$k}}">{{$v}}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    <div>
                                        @foreach ($field_info['source'] as $k => $v)
                                        <input type="radio" id="{{$field}}" @if($model->$field==$k) checked @endif name="{{$field}}" value ="{{$k}}"/>{{$v}}
                                        @endforeach
                                    </div>
                                    ...
                                    <label>{{$field_info['show']}}</label>
                                    <div>
                                    <input id="{{$field}}" value="{{$model->$field}}" name="uploadimg" type="file" multiple class="file see">
                                    </div>
                                    <input type="hidden" name="{{$field}}" value="{{$model->$field}}">
                                    <!-- </div> -->
                                    @elseif($field_info['type']=='checkbox')<!--radio框 未完-->
                                    <label>{{$field_info['show']}}</label>
                                    <div>
                                        @foreach ($field_info['source'] as $k => $v)
                                        <input type="checkbox" name="{{$field}}[]" id="{{$field}}[]" @if($field_info['vtype']=='related') @if(in_array($k,$model->$field->lists('id'))) checked @endif @endif value ="{{$k}}"/>{{$v}}
                                        @endforeach
                                    </div>
                                    @elseif($field_info['type']=='textarea')
                                    <label>{{$field_info['show']}}</label>
                                    <script type="text/plain" id="{{$field}}" name="{{$field}}" style="width:100%;height:150px;" class="editor_textarea">{!!$model->$field!!}</script>
                                    @else
                                    <label>{{$field_info['show']}}</label>
                                    <input name="{{$field}}" id="{{$field}}" type="text" class="form-control"
                                      value="{{$model->$field}}" @if(@$field_info['default'])disabled="true"@endif/>
                                    @endif
                                    @else
                                    <!-- <div class="form-group"> -->
                                    <label>{{$field_info['show']}}</label>
    ...
    setTimeout(function () {
        $(".editor_textarea").each(function(k,v){
            UE.getEditor(''+$(v).attr('id')+'', {
                toolbars: [
                    ['bold', 'italic', 'underline', 'fontfamily', 'fontsize', 'simpleupload', 'insertcode']
                ]
            });
        });
    },0);
