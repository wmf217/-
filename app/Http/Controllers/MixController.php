...
class MixController extends Controller {
    ...
    protected $_title='';//标题
    protected $_order=['id', 'desc'];//排序
    protected $_index='';//索引
    protected $_p_index='';//父级索引
    ...
    protected $_handle_btn=[
        'create'=>['way'=>'nomal','show'=>'新增','noway'=>'dist','require'=>true],//注意php eval
        'edit'=>['way'=>'nomal','noway'=>'dist','show'=>'编辑','require'=>true],
        'destroy'=>['way'=>'nomal','noway'=>'dist','show'=>'删除','require'=>true],
        'show'=>['way'=>'nomal','show'=>'查看','require'=>false]
    ];//按键权限,操作方式
    /**基础配置end**/
    ...
    protected $_single_btn=false;//单一操作
    ...
    protected $_buttons=[];//自定义右上角操作按钮
    protected $_tabs=[];//自定义tab(ztree,iframe)
    protected $_magnifying=['type'=>'iframe','show'=>'查看','require'=>false,'url'=>'/home'];//自定义点击行
    /**自定义配置end**/
    //构造函数
    public function __construct()
    {
        ...
        $this->_model_path = "App\\".$this->_model;
        // 导航
        ...
        View::share('index',$this->_index);
        View::share('p_index',$this->_p_index);
    ...
    public function index($matrix_id)
    {
        ...
        $builder = $model->orderBy($this->_order[0], $this->_order[1]);
        ...
        return View::make('mix.index', [
             'models' => $models, 'matrix_id'=>$matrix_id, 'fields_show'=>$fields_show, 'handle_btn'=>$this->_handle_btn,
             'list_buttons' => $this->_list_buttons, 'buttons' => $this->_buttons, 'matrixname' => $this->_matrix_name, 'magnifying' => $this->_magnifying,
             'single_btn' => $this->_single_btn
        ]);
    ...
    public function show($matrix_id, $id){
        $fields_edit = array(); // todo 暂时和edit用一个
        foreach ($this->_fields_detail as $field) {
            $fields_edit[$field] = $this->_fields_all[$field];
        }
        $builder = new $this->_model_path;
        $model = $builder->find($id);
        return View::make('mix.show', compact('model','matrix_id',
        'fields_edit','matrix_id'));
    }
    public function create($matrix_id)
    {
        ...
        $tabs=$this->_tabs;
        $pic=$this->_pic;
        return View::make('mix.create', compact('matrix_id','tabs',
    ...
    public function edit($matrix_id, $id)
    {
        ...
        return View::make('mix.edit', compact('model','matrix_id','tabs',
    ...
    public function beforestore(&$model){return $this->jsData('');}//保存前执行函数
    public function afterstore(&$model) {return $this->jsData('');}//保存回调函数
    public function beforeupdate(&$model) {return $this->jsData('');}//编辑前执行函数
    public function afterupdate(&$model) {return $this->jsData('');}//编辑回调函数
    public function beforedestroy(&$model) {return $this->jsData('');}//删除前执行函数
    public function afterdestroy(&$model) {return $this->jsData('');}//删除回调函数
}
